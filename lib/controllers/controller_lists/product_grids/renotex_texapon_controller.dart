...
class RenotexTexaponController extends GetxController{
  final RenotexTexaponRepo renotexTexaponRepo;

  RenotexTexaponController({required this.renotexTexaponRepo});

  List<ProductModel> _renotexTexaponList = [];
  List<ProductModel> get renotexTexaponList => _renotexTexaponList;
  ...
  Future<void> getRenotexTexaponList() async {
    Response response = await renotexTexaponRepo.getRenotexTexaponList();
    if(response.statusCode == 200) {
      _renotexTexaponList = [];
      _renotexTexaponList.addAll(Product.fromJson(response.body).products);
      _isLoaded = true;
      print(_renotexTexaponList);
